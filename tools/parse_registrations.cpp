...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::cerr << std::format("Usage: {} <header.hpp> <outputDir> [--split | -s]\n", commandPath.string());
        return 1;
    }
    ...
    if ((argc > 3) && ((std::strcmp(argv[3], "--split") == 0) || (std::strcmp(argv[3], "-s") == 0))) {
    ...
    while (std::getline(fin, line)) {
        ...
        if (auto combos = cartesianProduct(info.expansions); !expansionsSplit || combos.empty()) {
            ...
            fout << std::format("#include <gr_registry.hpp>\n#include \"{0}\" // for details: {0}:1\n\n", headerPath.string());
